...
"""
distance function:
returns sum of grid-wise distance
...
class MyCreature:
    ...
    def __init__(self):
        """
        You should initialise self.chromosome member variable here (whatever you choose it to be
            - a list/vector/matrix of numbers - and initialise it with some random values
        """
    ...
    def AgentFunction(self, percepts):
        ...
        The 'actions' variable must be returned and it must be a 5-dim numpy vector or a list with 5 numbers.

        The index of the largest numbers in the 'actions' vector/list is the action taken with the following interpretation:
            0 - move left
            1 - move up
            2 - move right
            3 - move down
        ...
        Different 'percepts' values should lead to different 'actions'.  This way the agent reacts differently to different situations.
        Different 'self.chromosome' should lead to different 'actions'.  This way different agents can exhibit different behaviour.
        """
...
def newGeneration(old_population):
    ...
    """
    Here you should modify the new_creature's chromosome by selecting two parents (based on their fitness) 
        and crossing their chromosome to overwrite new_creature.chromosome
    ...
    for n in range(N):
        if n < lowerLim:
            ...
            # takes the part of parent1 before the split point, and the part of parent2 that comes after split point
            chromoRaw = list(parent1Chromo) + list(parent2Chromo)
            ...
            for i in range(len(stdevs)):
                ...
                if ((stdevs[i]/2) < breakoutThresh) & (oddsMutation < probabilityOfBreakOut):
                    mutation = random.randint(-int(abs(parent1.mutationRate)), int(abs(parent1.mutationRate))) # abs added as value can be negative, and it crashes
                new_creature.chromosome[i] = int(chromoRaw[i] + mutation)  # allows for genetic diversity between children
            retainedIndex += 1

            '''
            
            ## generic random value based mutation mode, this is defunct
